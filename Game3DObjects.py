...
class Brick(Cube):
    def __init__(self, position, width, height, color):
        ...
        self.corner_1 = Point(self.pos.x - self.w / 2, self.pos.y - self.h /2, self.l)
        self.corner_2 = Point(self.pos.x - self.w / 2, self.pos.y + self.h /2, self.l)
        self.corner_3 = Point(self.pos.x + self.w / 2, self.pos.y + self.h /2, self.l)
        self.corner_4 = Point(self.pos.x + self.w / 2, self.pos.y - self.h /2, self.l)

        self.sides = []
        self.sides.append(LineObstacle(self.corner_1, self.corner_2))
        self.sides.append(LineObstacle(self.corner_1, self.corner_3))
        self.sides.append(LineObstacle(self.corner_4, self.corner_2))
...
class LineObstacle(Line):
    def __init__(self, point_1, point_2):
        ...
        self.normal_vector = Vector(-(self.point_2.y - self.point_1.y), self.point_2.x - self.point_1.x, 1)
    ...
    def collision(self, c_position, radius, c_motion, delta_time):
        ...
        vector = Vector(c_position.x - closestPointOnLine.x, c_position.y - closestPointOnLine.y, 1)
        vector.normalize()

        traversal = vector * radius
        closestPointOnCircle = Point(c_position.x - traversal.x, c_position.y - traversal.y, 0) 
        # print(f"cPC: {closestPointOnCircle}")
        # print(f"cPL: {closestPointOnLine}")
